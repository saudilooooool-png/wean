...
const navLinks = [
  ...
  { href: '/places', label: 'الأماكن', icon: Map },
...
function SidebarContent({ connected, pathname, onNavigate }: SidebarContentProps) {
    ...
    <div className="flex flex-col h-full">
      ...
      <div className="px-4 py-3 border-b border-slate-100">
        ...
        >
          ...
          <span
            className={`w-2 h-2 rounded-full me-auto flex-shrink-0 ${
                ...
                >
                  <Icon
                    ...
                    className={`flex-shrink-0 nav-icon ${
                      active ? 'text-green-500' : 'text-slate-400 group-hover:text-slate-600'
                    }`}
                  />
                  <span className="font-medium">{label}</span>
                  {active && (
                    <span
                      className="w-1.5 h-1.5 rounded-full me-auto"
      ...
      <div className="p-4 border-t border-slate-100">
        <div className="text-xs text-slate-400 text-center">
          ...
          <div className="mt-1">الإصدار 2.0.0</div>
        </div>
      </div>
    </div>
  )
  ...
  return (
    <>
      ...
      <aside className="hidden lg:flex flex-col w-64 bg-white border-l border-slate-200 h-screen sticky top-0 overflow-hidden flex-shrink-0">
        <SidebarContent connected={connected} pathname={pathname} />
      </aside>
